...
import { defaultNEATGenomeOptions, type NEATGenomeOptions } from '../src/NEATGenomeOptions';
import type { NEATState } from '../src/NEATState';

import { DefaultNEATGenome, type DefaultNEATGenomeData } from '../src/DefaultNEATGenome';
import { NodeType, type NodeData } from '@neat-js/core';
import type { GenomeData } from '@neat-js/core';
import type { Stats } from '@neat-js/core';
import type { NEATNode } from '../src/NEATNode';
import type { NEATLink } from '../src/NEATLink';
import { createConfig } from '../src/createConfig';
import { defaultNEATConfigOptions } from '@neat-js/core';
import type { LinkData } from '@neat-js/core';
...
describe('DefaultNEATGenome class', () => {
  ...
  let state: NEATState;
  let defaultData: GenomeData<NEATNode, NEATLink, Stats, NEATGenomeOptions, DefaultNEATGenome>
  ...
  beforeEach(() => {
    ...
    createSeasonedGenome = () => {
      ...
      const genome = createGenome(config, genomeOptions, state);
      for (let i = 0; i < 50; i++) {
        genome.mutate();
      }
      return genome;
  ...
  describe('DefaultNEATGenome constructor', () => {
    test('should correctly initialize', () => {
      const genome = createGenome(configProvider, genomeOptions, state);
    ...
    describe('DefaultNEATGenome toJSON', () => {
      let inputNode: NodeData<null, null>;
      let node1: NodeData<null, null>;
      let outputNode: NodeData<null, null>;
      let hiddenNodes: NodeData<null, null>[]
      let links: LinkData<null, null>[]
      let data: DefaultNEATGenomeData;

      beforeEach(() => {
        inputNode = { id: 0, type: NodeType.Input, config: null, state: null };
        node1 = { id: 1, type: NodeType.Hidden, config: null, state: null };
        outputNode = { id: 2, type: NodeType.Output, config: null, state: null };
        hiddenNodes = [node1]
        // FIXME: these links are garbage
        links = [
          { from: inputNode, to: node1, weight: 1, innovation: 1, config: null, state: null },
          { from: node1, to: outputNode, weight: 1, innovation: 1, config: null, state: null }
        ]
        data = {
          ...defaultData,
          hiddenNodes,
      ...
      test('should properly hydrate from given data', () => {
        const genome = createGenome(configProvider, genomeOptions, state, data);
  ...
  describe('mutationAddLink', () => {
    ...
    test('should add a new link with fresh genome', () => {
      ...
      expect(newLink.weight).to.be.at.least(-x).and.to.be.at.most(x);
    ...
    describe('mutationAddNode', () => {
      test('should add a new node with fresh genome', () => {
        ...
        const hiddenNode = Array.from(genome.hiddenNodes.values())[0] as NEATNode;
      ...
      test('should correctly configure links, hiddenNodes and connections', () => {
        ...
        // ]);
        expect(genome.connections.getAllConnections()).to.be.length(2);
  ...
  describe('DefaultNEATGenome distance function', () => {
    test('should return zero for identical genomes', () => {
      const genome1 = createGenome(configProvider, genomeOptions, state);
      const genome2 = createGenome(configProvider, genomeOptions, state);
  ...
  describe('DefaultNEATGenome crossover function', () => {
    ...
    test('should correctly perform crossover with overlapping links and nodes', () => {
      ...
      expect(child.hiddenNodes.size).toBeGreaterThanOrEqual(parent1.hiddenNodes.size);
    });
  })
