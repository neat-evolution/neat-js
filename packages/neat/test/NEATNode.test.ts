...
import type { NEATNode } from './NEATNode';
...
describe('NEATNode class', () => {
  describe('NEATNode methods', () => {
    ...
    beforeEach(() => {
      node = createNode(NodeType.Input, 1, null, null); node
    })
    ...
    test('should return 0', () => {
      expect(node.distance(node)).toEqual(0);
    })

    test('should return string', () => {
      expect(node.toString()).toEqual('Input[1]');
    })
