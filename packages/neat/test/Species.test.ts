...
import { Species, type GenomeDataParser } from '@neat-js/evolution';
import type { DefaultNEATGenome, DefaultNEATGenomeData } from '../src/DefaultNEATGenome';
import { defaultNEATGenomeOptions, type NEATGenomeOptions } from '../src/NEATGenomeOptions';
import { defaultSpeciesOptions, type SpeciesOptions } from '@neat-js/evolution';
...
import type { NEATState } from '@neat-js/core';
...
describe('Species class', () => {
  ...
  let parseGenomeData: GenomeDataParser<NEATGenomeOptions, DefaultNEATGenome, DefaultNEATGenomeData>
  ...
  beforeEach(() => {
    ...
    createSeasonedGenome = () => {
      ...
      const genome = createGenome(createConfig(configOptions), options, stateProvider);
      for (let i = 0; i < 50; i++) {
        genome.mutate();
      }
      return genome;
  ...
  describe('Species constructor', () => {
    test('should correctly initialize', () => {
      ...
      const species = new Species<DefaultNEATGenome, DefaultNEATGenomeData>(options, parseGenomeData);
  ...
  describe('Species push', () => {
    test('should add organism to species', () => {
      const species = new Species<DefaultNEATGenome, DefaultNEATGenomeData>(defaultSpeciesOptions, parseGenomeData);
