...
import type { NEATLink } from './NEATLink';
import { NodeType, type NodeRef } from '@neat-js/core';
...
describe('NEATLink class', () => {
  describe('NEATLink methods', () => {
    ...
    test('should return string', () => {
      expect(link.toString()).toEqual('Input[0] -> Output[1]');
    })
  ...
  describe('NEATLink crossover', () => {
    ...
    test('should throw an error for mismatched from node', () => {
      link2 = createLink({ type: NodeType.Hidden, id: 3 }, to, 0.7, 1, null, null);
      expect(() => {
    ...
    test('should throw an error for mismatched to node', () => {
      link2 = createLink(from, { type: NodeType.Hidden, id: 3 }, 0.7, 1, null, null);
      expect(() => {
        link1.crossover(link2, 10, 5);
      }).toThrowError('Mismatch in crossover');
    });
  ...
  describe('NEATLink toJSON', () => {
    ...
    test('should create a link from JSON data', () => {
      ...
      link = createLink(data.from, data.to, data.weight, data.innovation, data.config, data.state)
