...
import { createA2CStepAgent } from '../src/features/actor-critic/createA2CStepAgent.js'
...
function createMockTrainableExecutor(): TrainableExecutor & {
  ...
  const outputs = new Map<string, Float64Array>([
    ['1,0', new Float64Array([0.7, 0.3, 0.2])],
    ['0,1', new Float64Array([0.4, 0.6, 0.5])],
  ])
...
describe('createA2CStepAgent', () => {
  it('trains from a collected rollout batch', () => {
    const executor = createMockTrainableExecutor()
    const agent = createA2CStepAgent(
      executor,
      {
        learningRate: 0.1,
        actionCount: 2,
    ...
    expect(executor.backwardCalls).toHaveLength(2)
  })
})
