...
export const createReproducer: ReproducerFactory<any, any, undefined> = <
  ...
  P extends Population<any, any, any, any, any, any, any, G, any, undefined>
>(
  population: P
): Reproducer<G> => {
  const rng = threadRNG()
  return {
    copyElites: async (speciesIds: number[]) => {
      const organisms: Array<Organism<G>> = []
      for (const i of speciesIds) {
        const species = population.species.get(i) as Species<any, any, G>
