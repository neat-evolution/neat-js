...
async function handleInit({
  ...
  self.postMessage(createAction(ActionType.INIT_SUCCESS, null))
}
...
async function handleEvaluate(serializedPhenotypeData: EvaluatePayload) {
  ...
  self.postMessage(createAction(ActionType.EVALUATION_RESULT, fitnessData))
}
...
self.addEventListener('message', (event: MessageEvent<Action<unknown>>) => {
