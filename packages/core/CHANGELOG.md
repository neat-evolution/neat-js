# Change Log - @neat-evolution/core

<!-- This log was last generated on Tue, 10 Mar 2026 23:01:45 GMT and should not be manually modified. -->

<!-- Start content -->

## 0.5.0

Tue, 10 Mar 2026 23:01:45 GMT

### Minor changes

- add AnyErasedAlgorithm and AnyErasedGenome type aliases; replace innovation-key hashing with node-pair-based split innovation; refine AlgorithmContext generics and optimize Connections; introduce AlgorithmContext to replace verbose generic type params (github@heygrady.net)

## 0.3.9

Fri, 27 Feb 2026 09:07:42 GMT

### Patches

- optimize CoreGenome distance and crossover logic; implement static object pooling for Connections and incremental cache updates (github@heygrady.net)

## 0.3.7

Thu, 26 Feb 2026 22:05:36 GMT

### Patches

- apply Biome formatting (github@heygrady.net)

## 0.3.5

Tue, 02 Dec 2025 18:45:22 GMT

### Patches

- chore: update quick-lru to ^7.3.0 (github@heygrady.net)

## 0.3.3

Fri, 28 Nov 2025 20:11:15 GMT

### Patches

- Upgrade dependencies and update scripts (github@heygrady.net)
- Bump @neat-evolution/utils to v0.2.5

## 0.3.0

Mon, 18 Dec 2023 17:46:28 GMT

### Minor changes

- feat: remove innovation log, use hash key for node and link ids (github@heygrady.net)
- Bump @neat-evolution/utils to v0.2.2

## 0.2.1

Wed, 29 Nov 2023 17:31:33 GMT

### Patches

- fix: format (github@heygrady.net)
- Bump @neat-evolution/utils to v0.2.1

## 0.2.0

Thu, 09 Nov 2023 08:12:03 GMT

### Minor changes

- feat: rework types, rename NEATGenome to CoreGenome (github@heygrady.net)
- Bump @neat-evolution/utils to v0.2.0
