...
export const isNode = <N extends CoreNode>(node: N | NodeRef): node is N => {
  return (node as N).toFactoryOptions != null
}

export const toNodeRef = <N extends CoreNode>(node: N | NodeRef): NodeRef => {
  if (isNode(node)) {
...
export const toNodeRefTuple = <N extends CoreNode>(
  node: N | NodeRef
): NodeRefTuple => {
  if (isNode(node)) {
