# Change Log - @neat-evolution/worker-threads

<!-- This log was last generated on Tue, 10 Mar 2026 23:01:45 GMT and should not be manually modified. -->

<!-- Start content -->

## 0.4.6

Tue, 10 Mar 2026 23:01:45 GMT

### Patches

- adopt AlgorithmContext-based generics (github@heygrady.net)

## 0.4.4

Thu, 26 Feb 2026 22:05:36 GMT

### Patches

- apply Biome formatting and fix tsconfig (github@heygrady.net)

## 0.4.2

Sat, 29 Nov 2025 18:00:45 GMT

### Patches

- Fix browser Worker type definition (github@heygrady.net)

## 0.4.0

Fri, 28 Nov 2025 20:11:15 GMT

### Minor changes

- Update for worker-actions compatibility (github@heygrady.net)

## 0.2.2

Mon, 18 Dec 2023 17:46:28 GMT

### Patches

- feat: remove innovation log, use hash key for node and link ids (github@heygrady.net)

## 0.2.1

Wed, 29 Nov 2023 17:31:33 GMT

### Patches

- fix: format (github@heygrady.net)

## 0.2.0

Thu, 09 Nov 2023 08:12:03 GMT

### Minor changes

- feat: worker-threads (github@heygrady.net)
