...
The `@neat-evolution/hyperneat` package implements the HyperNEAT
(Hypercube-based NeuroEvolution of Augmenting Topologies) algorithm. HyperNEAT
is an extension of NEAT that uses Compositional Pattern Producing Networks
(CPPNs) to evolve the connectivity patterns of a larger, fixed-topology neural
network called a "substrate." This approach allows for the evolution of highly
complex and regular neural network structures, often exhibiting properties like
...
- **Implement HyperNEAT:** Provide a robust implementation of the HyperNEAT
  algorithm, enabling the evolution of substrate-based neural networks.
- **Leverage CPPNs:** Utilize CPPNs (from `@neat-evolution/cppn`) as the
  "indirect encoding" mechanism to generate the weights and connections of the
  substrate network.
- **Evolve Complex Architectures:** Facilitate the evolution of neural networks
  with intricate and spatially organized connectivity, which can be beneficial
  for tasks with inherent spatial relationships (e.g., vision, robotics).
- **Provide Substrate Management:** Define and manage the substrate network,
  including its nodes, connections, and the process of querying the CPPN to
  determine its structure.

## How it Fits into the Ecosystem

The `hyperneat` package is a significant extension of the core NEAT and CPPN
functionalities. It integrates with other packages in the `neat-js` monorepo as
follows:

- **`@neat-evolution/core`**: Provides fundamental NEAT concepts and data
  structures.
- **`@neat-evolution/neat`**: HyperNEAT builds upon the evolutionary mechanisms
  and genome structure defined in the `neat` package.
- **`@neat-evolution/cppn`**: This is a crucial dependency, as CPPNs are used as
  the "brain" that designs the substrate network. The `hyperneat` package will
  query a CPPN genome to determine the properties of the substrate's
  connections.
- **`@neat-evolution/evolution`**: The `hyperneat` function orchestrates the
...
- **`@neat-evolution/evaluator`**: Requires an `Evaluator` instance to assess
  the fitness of evolved HyperNEAT (CPPN) genomes by evaluating the performance
  of their generated substrate networks.
- **`@neat-evolution/demo`**: The `demo` package uses
  `@neat-evolution/hyperneat` to demonstrate the HyperNEAT algorithm in action.

## Installation

To install the `@neat-evolution/hyperneat` package, use the following command:
...
The `hyperneat` package exposes several important classes, interfaces, and
functions:
...
- **`hyperneat(...)` function**: The main entry point for running the HyperNEAT
  ...
  up and execute the evolutionary process. Note that the "genome" being evolved
...
To run the HyperNEAT algorithm, you typically call the `hyperneat` function,
providing it with the necessary factories and options. This function will then
manage the evolutionary process of CPPN genomes, which in turn design the
substrate networks.
